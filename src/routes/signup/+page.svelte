...
>
  ...
  <!-- Ambient glow blobs -->
  <div class="absolute top-1/4 right-1/3 w-96 h-96 rounded-full pointer-events-none" style="background: radial-gradient(circle, rgba(52,211,153,0.05) 0%, transparent 70%);"></div>
  <div class="absolute bottom-1/3 left-1/4 w-80 h-80 rounded-full pointer-events-none" style="background: radial-gradient(circle, rgba(251,191,36,0.04) 0%, transparent 70%);"></div>
  ...
  <div class="absolute bottom-0 left-0 right-0 pointer-events-none" style="opacity: 0.12;">
    <svg viewBox="0 0 1440 200" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
      <path d="M0,200 L0,130 L100,70 L200,110 L340,40 L460,90 L580,25 L700,75 L820,15 L940,60 L1060,30 L1180,70 L1300,45 L1440,85 L1440,200 Z" fill="#4ade80" />
    </svg>
  </div>
  ...
  <div class="relative w-full max-w-md">
    ...
    {#if !showVerification}
      ...
      <div class="text-center mb-8">
        ...
        <h1 class="text-3xl font-black tracking-tight" style="color: #f5f0e8;">Start your journey</h1>
        <p class="mt-1 text-sm" style="color: rgba(245,240,232,0.5);">Create your free Adventure Spark account</p>
      </div>
      ...
      <div class="rounded-2xl p-8 space-y-6" style="background: rgba(245,240,232,0.05); border: 1px solid rgba(245,240,232,0.1); backdrop-filter: blur(12px);">
        ...
        >
          <!-- Name -->
          <div>
            <label for="name" class="block text-xs font-semibold mb-1.5 tracking-wide uppercase" style="color: rgba(245,240,232,0.5);">Full name</label>
            <input
              id="name"
              name="name"
              type="text"
          ...
          <!-- Password -->
          <div>
            <label for="password" class="block text-xs font-semibold mb-1.5 tracking-wide uppercase" style="color: rgba(245,240,232,0.5);">Password</label>
            <div class="relative">
              <input
                id="password"
                name="password"
                    ...
                    <div class="rounded-xl p-3 space-y-2" style="background: rgba(12,20,14,0.97); border: 1px solid rgba(245,240,232,0.15); backdrop-filter: blur(12px);">
                      ...
                      {#each [
                        ...
                        <div class="flex items-center gap-2">
                          {#if req.met}
                            <svg class="h-3.5 w-3.5 flex-shrink-0" fill="none" stroke="#34d399" stroke-width="2.5" viewBox="0 0 24 24">
                              <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                            </svg>
                          {:else}
                            <div class="h-3.5 w-3.5 flex-shrink-0 rounded-full" style="border: 1.5px solid rgba(245,240,232,0.2);"></div>
                          {/if}
                          <span class="text-xs" style="color: {req.met ? '#6ee7b7' : 'rgba(245,240,232,0.4)'};">{req.label}</span>
                        </div>
                      {/each}
                    </div>
                  </div>
          ...
          <!-- Confirm password -->
          <div>
            <label for="confirmPassword" class="block text-xs font-semibold mb-1.5 tracking-wide uppercase" style="color: rgba(245,240,232,0.5);">Confirm password</label>
            <div class="relative">
              <input
                id="confirmPassword"
    ...
    {:else}
      <!-- Verification state -->
      <div class="text-center mb-8">
        <div class="inline-flex items-center justify-center w-16 h-16 rounded-full mb-4" style="background: rgba(251,191,36,0.12); border: 1px solid rgba(251,191,36,0.25);">
          <svg class="w-8 h-8" viewBox="0 0 24 24" fill="none" stroke="#fbbf24" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
        ...
        <p class="mt-2 text-sm" style="color: rgba(245,240,232,0.5);">
          We sent a code to <span class="font-semibold" style="color: rgba(245,240,232,0.8);">{userEmail}</span>
        </p>
      </div>
      ...
      <div class="rounded-2xl p-8 space-y-6" style="background: rgba(245,240,232,0.05); border: 1px solid rgba(245,240,232,0.1); backdrop-filter: blur(12px);">
        ...
        >
          ...
          <div>
            <label for="code" class="block text-xs font-semibold mb-1.5 tracking-wide uppercase" style="color: rgba(245,240,232,0.5);">Verification code</label>
            <input
              id="code"
              name="code"
              type="text"
            ...
            />
            <p class="mt-1.5 text-xs text-center" style="color: rgba(245,240,232,0.35);">Enter the 6-digit code from your email</p>
          </div>
        ...
        <div class="relative">
          ...
          <div class="relative flex justify-center text-xs">
            <span class="px-3" style="background: rgba(8,21,16,0.95); color: rgba(245,240,232,0.35);">Didn't receive the code?</span>
          </div>
        </div>
