<script lang="ts">
  ...
  const editableFields = [
    ...
    { key: "reservationInfo", label: "Reservation Info", type: "textarea" },
    { key: "costPerNight", label: "Cost Per Night", type: "number" },
    { key: "baseFee", label: "Base Fee", type: "number" },
    {
      key: "operatingSeasonStart",
      label: "Operating Season Start",
      type: "text",
    },
    { key: "operatingSeasonEnd", label: "Operating Season End", type: "text" },
    {
      key: "petPolicy",
      label: "Pet Policy",
      type: "select",
      options: ["allowed", "not_allowed", "restricted"],
    },
    {
      key: "siteType",
      label: "Site Type",
      type: "select",
      options: ["public", "private", "public_private_partnership"],
    },
    {
      key: "firePolicy",
      label: "Fire Policy",
  ...
  function getCurrentValue(field: string): string {
    ...
    return String(campingSite[field] || "");
  }
    ...
    <div class="bg-white shadow rounded-lg p-6">
      <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">
          {isAdmin ? "Edit Camping Site" : "Suggest Edit"}
        </h1>
      ...
      >
        ...
        <div class="flex justify-end gap-3 pt-4 border-t">
          ...
          >
            ...
            {:else}
              {isAdmin ? "Update Camping Site" : "Submit Suggestion"}
            {/if}
          </button>
        </div>
      </form>
    ...
    {#if isAdmin}
      <div class="mt-6 bg-white shadow rounded-lg p-6 border border-red-200">
        ...
        <p class="text-sm text-gray-600 mb-4">
          Permanently delete this camping site and all associated images. This cannot be undone.
        </p>
        ...
        {:else}
          <p class="text-sm font-medium text-red-800 mb-3">
            Are you sure? This will permanently delete "{data.campingSite.name}" and all its images.
          </p>
          <form
            method="POST"
            action="?/delete"
