  ...
  <div class="relative overflow-hidden profile-banner -mt-16">
    ...
    <div class="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-16">
      <div class="flex items-center gap-5">
        <div class="w-20 h-20 rounded-2xl flex items-center justify-center text-white text-3xl font-black shadow-xl flex-shrink-0 border-2 border-white/20" style="background: linear-gradient(135deg, #059669, #34d399);">
          {initials}
        </div>
        <div>
          <h1 class="text-2xl sm:text-3xl font-black leading-tight mb-0.5" style="color: #f5f0e8;">{displayName}</h1>
          <p class="text-sm" style="color: rgba(245,240,232,0.45);">{data.user.email}</p>
          {#if data.user.role === "admin"}
            <span class="inline-flex items-center mt-2 px-2.5 py-0.5 rounded-full text-xs font-bold" style="background: rgba(251,146,60,0.15); border: 1px solid rgba(251,146,60,0.35); color: rgb(251,146,60);">
              Admin
            </span>
          {/if}
        </div>
        ...
        <Tabs {tabs} bind:activeTab>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="rounded-xl p-5 bg-stone-50 border border-stone-100">
                  <div class="flex items-center gap-2 mb-3">
                    ...
                    <p class="text-xs font-bold text-stone-400 uppercase tracking-widest">Display Name</p>
                  </div>
          ...
          {:else if activeTab === "security"}
            <div class="pb-6">
              ...
              <p class="text-sm text-stone-500 mb-6">Update your password to keep your account secure.</p>
              ...
              >
                <div>
                  <label for="currentPassword" class="block text-xs font-bold text-stone-500 uppercase tracking-widest mb-2">Current Password</label>
                ...
                <div>
                  <label for="newPassword" class="block text-xs font-bold text-stone-500 uppercase tracking-widest mb-2">New Password</label>
                  <input type="password" id="newPassword" name="newPassword" required minlength="8"
                    class="w-full px-4 py-3 border border-stone-200 rounded-xl bg-stone-50 text-stone-900 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-transparent focus:bg-white transition-all outline-none" />
                  <p class="mt-1.5 text-xs text-stone-400">At least 8 characters</p>
                </div>
                <div>
                  <label for="confirmPassword" class="block text-xs font-bold text-stone-500 uppercase tracking-widest mb-2">Confirm New Password</label>
